...
import { LayoutDashboard, Users, Map, Settings, BarChart2, LogOut } from 'lucide-react';
  ...
  return (
    <aside className="w-64 bg-white dark:bg-[#111827] border-r border-slate-200 dark:border-slate-800 flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20"
        >
          <LogOut className="w-5 h-5 transition-transform duration-200 group-hover:translate-x-0.5 text-rose-500" />
          Log Out
        </button>
      </nav>
      ...
      <div className="p-4 border-t border-slate-100 dark:border-slate-800">
        <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <p className="text-xs font-semibold text-slate-900 dark:text-white truncate">Love Maggo</p>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">Software Engineer</p>
          </div>
        </div>
      </div>
    </aside>
